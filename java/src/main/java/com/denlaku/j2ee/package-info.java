 ...
 *         J2EE就是基于JAVA技术的一系列标准. J2EE是Java2平台企业版(Java 2Platform Enterprise
 *         Edition),核心是一组技术规范与指南,其中所包含的各类组件,服务架构和技术层次,都有共同的标准及规格,让各种依云J2EE架构的不同平台之间,存在良好的兼容性.
 * 
 *         J2EE的13种规范是什么? 
 *         1.JDBC(JavaDatabase Connectivity) JDBC是以统一方式访问数据库的API.
 *         它提供了独立于平台的数据库访问,也就是说,有了JDBC
 *         API,我们就不必为访问Oracle数据库专门写一个程序,为访问Sybase数据库又专门写一个程序等等,只需要用JDBC
 *         API写一个程序就够了,它可以向相应数据库发送SQL调用.JDBC是Java应用程序与各种不同数据库之间进行对话的方法的机制.简单地说,它做了三件事:与数据库建立连接--发送操作数据库的语句--处理结果.
 * 
 ...
 *         利用JNDI的命名与服务功能可满足企业级API对命名与服务的访问,诸如EJB,JMS,JDBC
 *         2.0以及IIOP上的RMI通过JNDI来使用CORBA的命名服务. JNDI和JDBC类似,都是构建在抽象层上.因为:
 ...
 * 
 *         4.RMI(Remote MethodInvoke) 是一组用户开发分布式应用程序的API.
 *         这一协议调用远程对象上的方法使用了序列化的方式在客户端和服务器之间传递数据,使得原先的程序在同一操作系统的方法调用,变成了不同操作系统之间程序的方法调用,即RMI机制实现了程序组件在不同操作系统之间的通信.它是一种被EJB使用的更底层的协议.
 *         RMI/JNI: RMI可利用标准Java本机方法接口与现有的和原有的系统相连接 RMI/JDBC:
 *         RMI利用标准JDBC包与现有的关系数据库连接 这就实现了与非Java语言的现有服务器进行通信.
 * 
 *         5.JavaIDL/CORBA(Common Object Request BrokerArchitecture)
 *         Java接口定义语言/公用对象请求代理程序体系结构
 *         在JavaIDL的支持下，开发人员可以将Java和CORBA集成在一起。他们可以创建Java对象并使之可在CORBA
 *         ORB中展开,或者他们还可以创建Java类并作为和其它ORB一起展开的CORBA对象的客户。后一种方法提供了另外一种途径，通过它Java可以被用于将新的应用和旧的系统相集成。
 *         CORBA是面向对象标准的第一步，有了这个标准，软件的实现与工作环境对用户和开发者不再重要，可以把精力更多地放在本地系统的实现与优化上。
 * 
 *         6.JSP(Java Server Pages) JSP页面=HTML+Java,其根本是一个简化的Servlet设计.
 ...
 *         定义了一种标准API,应用程序由此可以访问各种事务监控.它允许应用程序执行分布式事务处理--在两个或多个网络计算机资源上访问并且更新数据.JTA和JTS为J2EE
 *         平台提供了分布式事务服务.
 *         JTA事务比JDBC事务更强大,一个JTA事务可以有多个参与者,而一个JDBC事务则被限定在一个单一的数据库连接.
 * 
 *         11.JTS(JavaTransaction Service) JTS是CORBA
 *         OTS事务监控器的一个基本实现。JTS指定了一个事务管理器的实现（Transaction
 *         Manager），这个管理器在一个高级别上支持JTA规范，并且在一个低级别上实现了OMGOTS规范的Java映射。一个JTS事务管理器为应用服务器、资源管理器、standalone应用和通信资源管理器提供事务服务。
 * 
 *         12.JavaMail 用于访问邮件服务器的API,提供了一套邮件服务器的抽象类.
 * 
